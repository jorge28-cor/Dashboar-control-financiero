...
function renderDebts(container) {
  ...
  container.querySelectorAll('[data-debt-pay]').forEach(b=>b.addEventListener('click',()=>{
    const debt=state.debts.find(d=>d.id===b.dataset.debtPay);if(!debt)return;
    const a=parseFloat(prompt(`¿Cuánto pagaste a "${debt.name}"?\n(Pago mensual: ${fmt(debt.monthly)})`));
    if(!isNaN(a)&&a>0){debt.remaining=Math.max(debt.remaining-a,0);save();renderTab();renderHeader();toast(`${fmt(a)} registrado en ${debt.name}`);}
  }));
}
