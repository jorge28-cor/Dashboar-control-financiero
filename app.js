...
const CATEGORIES = [
  ...
  { id: 'deuda',           emoji: '💳', name: 'Pago deuda',    color: '#ff4d6d' },
  { id: 'inversion_out',   emoji: '📈', name: 'Inversión',     color: '#00d4ff' },
...
const INCOME_CATS = [
  ...
  { id: 'inversion_in', emoji: '📈', name: 'Retiro inv.', color: '#b57bee' },
...
const INVEST_BY_ZONE = {
  seguridad: [
    ...
    { id: 'fci',         emoji: '📂', name: 'FCI / MM' },
    { id: 'cuenta_rem',  emoji: '🏧', name: 'Cta. remunerada' },
    { id: 'cauciones',   emoji: '📋', name: 'Cauciones' },
  ],
  equilibrio: [
    { id: 'cedears',     emoji: '🇦🇷', name: 'CEDEARs' },
    { id: 'bonos',       emoji: '📜', name: 'Bonos / CER' },
    { id: 'etf',         emoji: '📊', name: 'ETFs' },
    { id: 'fondos_mix',  emoji: '🌐', name: 'Fondos mixtos' },
  ],
  crecimiento: [
...
function load() {
  try {
    const raw = localStorage.getItem('finanzas_v3') || localStorage.getItem('finanzas_v2') || localStorage.getItem('finanzas_state');
    if (!raw) return;
...
// Saldo líquido = ingresos - gastos (incluyendo pagos de deuda e inversiones como gastos)
function calcLiquidBalance() {
  ...
  // Pagos de deuda también se descuentan del saldo líquido
  const debtPaid = state.debtPayments.reduce((s,p) => s + p.amount, 0);
  // Inversiones activas se descuentan, retiros se suman (ya manejados como transacciones tipo ingreso cat inversion_in)
  const investedOut = state.investments.reduce((s,i) => s + i.cost, 0);
  return income - expense - debtPaid - investedOut;
}

// Patrimonio neto = saldo líquido + valor actual inversiones - deudas restantes
function calcNetWorth() {
  const liquid     = calcLiquidBalance();
  const investVal  = state.investments.reduce((s,i) => s + i.currentValue, 0);
  const totalDebt  = state.debts.reduce((s,d) => s + d.remaining, 0);
  return liquid + investVal - totalDebt;
}
...
function renderOverview(container) {
  ...
  const investGain  = totalInvest - state.investments.reduce((s,i) => s + i.cost, 0);
  ...
  row2.innerHTML = `
    ...
    <div class="kpi-card accent" style="cursor:pointer" id="quick-invest"><div class="kpi-label">📈 Inversiones</div><div class="kpi-value" style="color:var(--accent)">${fmt(totalInvest)}</div><div class="kpi-sub" style="color:${investGain>=0?'var(--green)':'var(--red)'}">${investGain>=0?'+':''}${fmt(investGain)} →</div></div>`;
...
function getRecommendations(txs, income, expense, savingRate) {
  ...
  if (state.investments.length === 0) recs.push({ icon: '📈', text: 'Sin inversiones. Registra tu portafolio para ver el triángulo.' });
  CATEGORIES.forEach(c => {
    const b = state.settings.budgets[c.id];
    if (b > 0) {
      const spent = txs.filter(t=>t.type==='gasto'&&t.category===c.id).reduce((s,t)=>s+t.amount,0);
...
function renderTransactions(container) {
  ...
  chips.innerHTML = filterDefs.map(f=>`<div class="chip ${state.txFilter===f.id?'active':''}" data-filter="${f.id}">${f.label}</div>`).join('');
...
function renderBudget(container) {
  ...
  summary.innerHTML = `
    ...
    <div style="display:flex;justify-content:space-between;margin-bottom:10px">
      <div><div style="font-size:11px;color:var(--muted)">Ingresos</div><div style="font-family:'JetBrains Mono',monospace;font-size:18px;color:var(--green)">${fmt(totalIncome)}</div></div>
      <div style="text-align:right"><div style="font-size:11px;color:var(--muted)">Gastos</div><div style="font-family:'JetBrains Mono',monospace;font-size:18px;color:var(--red)">${fmt(totalExpense)}</div></div>
    </div>
    ${expected>0?`<div style="background:var(--border);border-radius:99px;height:6px;margin-bottom:4px;overflow:hidden"><div style="width:${Math.min((totalExpense/expected)*100,100)}%;background:${totalExpense>expected?'var(--red)':'var(--accent)'};border-radius:99px;height:100%;transition:width .8s"></div></div><div style="font-size:11px;color:var(--muted)">${Math.round((totalExpense/expected)*100)}% del ingreso esperado</div>`:`<div style="font-size:12px;color:var(--muted)">Configura tu ingreso en ⚙️ para ver el análisis.</div>`}`;
...
function renderGoals(container) {
  ...
  addBtn.className = 'btn-add-goal'; addBtn.textContent = '+ Nueva meta';
...
function renderDebts(container) {
  ...
  if (state.debts.length > 0) {
    ...
    progCard.innerHTML = `
      ...
      <div style="background:var(--border);border-radius:99px;height:10px;overflow:hidden">
        <div style="width:${paidPct}%;background:linear-gradient(90deg,var(--green),var(--accent));border-radius:99px;height:100%;transition:width 1s"></div>
      </div>
      <div style="font-size:11px;color:var(--muted);margin-top:5px">Total original: ${fmt(totalOriginal)}</div>`;
    ...
    tip.className = 'card';
    tip.innerHTML = `
      <div class="card-title">💡 Estrategia avalancha</div>
      <div class="rec-item"><div class="rec-icon">🎯</div><div class="rec-text">Paga primero <strong>${highRate.name}</strong> (${highRate.rate}% tasa). Ahorrás más en intereses.</div></div>
  ...
  if (!state.debts.length) {
    listCard.innerHTML += `<div class="empty-state"><div class="empty-state-icon">🎉</div><div class="empty-state-text">Sin deudas registradas.</div></div>`;
  } else {
    state.debts.forEach(d => {
      ...
      const urgC   = d.rate>50 ? 'var(--red)' : d.rate>20 ? 'var(--yellow)' : 'var(--green)';
      const item   = document.createElement('div');
      ...
      item.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
          ...
          <div style="text-align:right">
            ...
            <div style="font-size:11px;background:${urgC}22;color:${urgC};padding:2px 7px;border-radius:99px;display:inline-block;margin-top:2px">${d.rate}% tasa</div>
          </div>
        </div>
        ...
        <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--muted);margin-bottom:10px">
          ...
          <span>${mLeft?`~${mLeft} meses`:''}</span>
        </div>
        <div class="goal-actions">
          <button class="btn-sm" data-debt-pay="${d.id}">💰 Pagar cuota</button>
  ...
  const addBtn = document.createElement('button');
  addBtn.className = 'btn-add-goal'; addBtn.textContent = '+ Agregar deuda';
  container.appendChild(addBtn);
  ...
  container.querySelectorAll('[data-debt-pay]').forEach(b=>b.addEventListener('click',()=>{ openPayDebtModal(b.dataset.debtPay); }));
  container.querySelectorAll('[data-debt-history]').forEach(b=>b.addEventListener('click',()=>{ showDebtHistory(b.dataset.debtHistory); }));
}
...
function openPayDebtModal(debtId) {
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      ...
      <div class="modal-sheet">
        ...
        <h2 class="modal-title" id="pay-debt-title">Pagar cuota</h2>
        <div id="pay-debt-info"></div>
        <div class="form-group"><label>Monto pagado ($)</label><div class="amount-input-wrap"><span class="currency-sign">$</span><input type="number" id="inp-pay-amount" placeholder="0.00" inputmode="decimal" /></div></div>
        <div class="form-group"><label>Fecha</label><input type="date" id="inp-pay-date" /></div>
        <div class="form-group"><label>Nota (opcional)</label><input type="text" id="inp-pay-note" placeholder="ej. Pago mínimo, abono extra..." /></div>
  ...
  document.getElementById('pay-debt-info').innerHTML = `
    <div class="pay-debt-summary">
      ...
      <div class="pay-debt-row"><span>Cuota mensual sugerida</span><span style="color:var(--yellow);font-family:'JetBrains Mono',monospace">${fmt(debt.monthly)}</span></div>
...
function confirmDebtPayment(debtId) {
  ...
  // Registrar como gasto en transacciones para que se descuente del saldo
  state.transactions.push({ id: uid(), type: 'gasto', amount, desc: `Pago: ${debt.name}`, category: 'deuda', date: date || todayStr() });
...
function showDebtHistory(debtId) {
  ...
  document.getElementById('hist-content').innerHTML = `
    <div style="background:var(--card);border-radius:12px;padding:14px;margin-bottom:16px">
      <div style="display:flex;justify-content:space-between;margin-bottom:6px"><span style="font-size:12px;color:var(--muted)">Total pagado</span><span style="font-family:'JetBrains Mono',monospace;color:var(--green)">${fmt(totalPaid)}</span></div>
      <div style="display:flex;justify-content:space-between"><span style="font-size:12px;color:var(--muted)">Saldo restante</span><span style="font-family:'JetBrains Mono',monospace;color:var(--red)">${fmt(debt.remaining)}</span></div>
    </div>
    ${payments.length===0
      ? `<div class="empty-state"><div class="empty-state-icon">📭</div><div class="empty-state-text">Sin pagos registrados.</div></div>`
      : payments.map(p=>`<div class="tx-item"><div class="tx-icon" style="background:var(--green-dim)">💳</div><div class="tx-info"><div class="tx-desc">${p.note||'Pago registrado'}</div><div class="tx-meta">${new Date(p.date+'T12:00:00').toLocaleDateString('es-MX',{day:'numeric',month:'short',year:'numeric'})} · Saldo restante: ${fmt(p.balanceAfter)}</div></div><div class="tx-amount income">-${fmt(p.amount)}</div></div>`).join('')}`;
  modal.classList.remove('hidden');
}
...
function renderInvestments(container) {
  ...
  triCard.innerHTML = `<div class="card-title">📐 Triángulo de Inversión</div><div style="font-size:11px;color:var(--muted);margin-bottom:14px">Perfil: <strong style="color:var(--accent)">${profileLabels[profile]}</strong></div>`;
  ...
  zones.forEach(z => {
    ...
    const bColor = Math.abs(diff)<=5 ? 'var(--green)' : Math.abs(diff)<=15 ? 'var(--yellow)' : 'var(--red)';
    const dLabel = diff===0 ? '✅ Ideal' : diff>0 ? `▲ +${diff}% sobre` : `▼ ${Math.abs(diff)}% bajo`;
    ...
    row.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px">
        ...
        <div style="font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--muted2)">${real}% <span style="color:var(--muted);font-size:10px">/ ideal ${ideal}%</span></div>
      </div>
      <div style="background:var(--border);border-radius:99px;height:8px;margin-bottom:4px;position:relative;overflow:visible">
        <div style="width:${real}%;background:${cfg.color};border-radius:99px;height:100%;transition:width 1s;box-shadow:0 0 6px ${cfg.color}55"></div>
        <div style="position:absolute;top:-4px;left:${ideal}%;width:2px;height:16px;background:rgba(255,255,255,0.3);border-radius:1px"></div>
      </div>
      <div style="display:flex;justify-content:space-between;font-size:10px">
        <span style="color:var(--muted)">${fmt(val)} · ${cfg.desc}</span>
        <span style="color:${bColor};font-weight:600">${totalValue>0?dLabel:''}</span>
      </div>`;
  ...
  if (totalValue > 0) {
    ...
    alertDiv.style.cssText = `background:${crec>ZONE_CONFIG.crecimiento.ideal[profile]+15?'var(--red-dim)':'var(--green-dim)'};border-radius:12px;padding:10px 12px;margin-top:8px;font-size:12px`;
    alertDiv.innerHTML = crec > ZONE_CONFIG.crecimiento.ideal[profile]+15
      ? `⚠️ <strong>Portafolio muy agresivo:</strong> ${crec}% en crecimiento. Considerá diversificar.`
      : seg > 70 ? `💤 <strong>Portafolio muy conservador:</strong> ${seg}% en zona segura. Tu dinero crece poco.`
      : `✅ <strong>Distribución saludable</strong> para tu perfil.`;
  ...
  if (!state.investments.length) {
    listCard.innerHTML += `<div class="empty-state"><div class="empty-state-icon">📈</div><div class="empty-state-text">Sin inversiones registradas.<br>CEDEARs, acciones, crypto, fondos...</div></div>`;
  } else {
    ...
    [...state.investments].sort((a,b)=>b.currentValue-a.currentValue).forEach(inv => {
      ...
      const gPct = inv.cost>0 ? ((gain/inv.cost)*100).toFixed(1) : 0;
      ...
      item.innerHTML = `
        ...
        <div style="text-align:right">
          ...
          <div style="font-size:11px;color:${gain>=0?'var(--green)':'var(--red)'}">${gain>=0?'+':''}${fmt(gain)} (${gPct}%)</div>
        </div>
  ...
  const addBtn = document.createElement('button');
  addBtn.className = 'btn-add-goal'; addBtn.textContent = '+ Agregar inversión';
  container.appendChild(addBtn);
  ...
  listCard.querySelectorAll('[data-inv-del]').forEach(b=>b.addEventListener('click',(e)=>{
    e.stopPropagation();
    if(confirm('¿Eliminar? Esto también devolverá el capital al saldo.')) {
      const inv = state.investments.find(i=>i.id===b.dataset.invDel);
      if (inv) {
        // Devolver el capital invertido como ingreso
        state.transactions.push({ id: uid(), type: 'ingreso', amount: inv.currentValue, desc: `Retiro: ${inv.name}`, category: 'inversion_in', date: todayStr() });
      }
      state.investments=state.investments.filter(i=>i.id!==b.dataset.invDel);
      save(); renderTab(); renderHeader(); toast('Inversión retirada — capital devuelto al saldo');
    }
  }));
  listCard.querySelectorAll('[data-inv-id]').forEach(item=>item.addEventListener('click',()=>{ state.editingInvestId=item.dataset.invId; openInvestModal(); }));
}
...
function renderFinancial(container) {
  ...
  const totalDebt   = state.debts.reduce((s,d)=>s+d.remaining,0);
  const totalAssets = Math.max(liquid,0) + totalInvest;
  ...
  title.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
      <div><div style="font-size:18px;font-weight:800">Estado Financiero</div><div style="font-size:12px;color:var(--muted)">${now.toLocaleDateString('es-MX',{month:'long',year:'numeric'})}</div></div>
      <div style="text-align:right"><div style="font-size:10px;color:var(--muted)">PATRIMONIO NETO</div><div style="font-family:'JetBrains Mono',monospace;font-size:20px;font-weight:700;color:${netWorth>=0?'var(--green)':'var(--red)'}">${fmt(netWorth)}</div></div>
    </div>`;
  ...
  balCard.innerHTML = `
    ...
    <div class="fs-row"><span>💵 Saldo líquido</span><span style="color:${liquid>=0?'var(--green)':'var(--red)'}">${fmt(liquid)}</span></div>
    <div class="fs-row"><span>📈 Inversiones (valor actual)</span><span style="color:var(--accent)">${fmt(totalInvest)}</span></div>
    ...
    ${state.debts.map(d=>`<div class="fs-row"><span>${DEBT_TYPES[d.type]?.emoji||'💳'} ${d.name}</span><span style="color:var(--red)">${fmt(d.remaining)}</span></div>`).join('')||`<div class="fs-row"><span style="color:var(--muted)">Sin deudas</span><span style="color:var(--green)">$0</span></div>`}
  ...
  // ── GRÁFICO BARRAS ACTIVOS/PASIVOS ──
  const barCard = document.createElement('div');
  barCard.className = 'card';
  const maxBar = Math.max(totalAssets, totalDebt, 1);
  const items = [
    { label:'Saldo líquido', val:Math.max(liquid,0), color:'var(--green)' },
    { label:'Inversiones',   val:totalInvest,          color:'var(--accent)' },
    { label:'Deudas',        val:totalDebt,             color:'var(--red)' },
  ];
  barCard.innerHTML = `
    ...
    ${items.map(i=>`
      <div style="margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:5px">
          ...
          <span style="font-family:'JetBrains Mono',monospace;color:${i.color}">${fmt(i.val)}</span>
        </div>
        <div style="background:var(--border);border-radius:99px;height:10px;overflow:hidden">
          <div style="width:${(i.val/maxBar)*100}%;background:${i.color};border-radius:99px;height:100%;transition:width 1s;box-shadow:0 0 6px ${i.color}55"></div>
        </div>
      </div>`).join('')}`;
  ...
  const byIncomeCat = {}, byExpenseCat = {};
  curTxs.filter(t=>t.type==='ingreso').forEach(t=>{ byIncomeCat[t.category]=(byIncomeCat[t.category]||0)+t.amount; });
  curTxs.filter(t=>t.type==='gasto').forEach(t=>{ byExpenseCat[t.category]=(byExpenseCat[t.category]||0)+t.amount; });
  const topInc = Object.entries(byIncomeCat).sort((a,b)=>b[1]-a[1]);
  const topExp = Object.entries(byExpenseCat).sort((a,b)=>b[1]-a[1]).slice(0,5);
  resCard.innerHTML = `
    <div class="card-title">📋 Resultados — ${now.toLocaleDateString('es-MX',{month:'long'})}</div>
    <div class="fs-section-label" style="color:var(--green)">INGRESOS</div>
    ${topInc.map(([cat,val])=>{ const c=INCOME_CATS.find(x=>x.id===cat)||{emoji:'💵',name:cat}; return `<div class="fs-row"><span>${c.emoji} ${c.name}</span><span style="color:var(--green)">${fmt(val)}</span></div>`; }).join('')||`<div class="fs-row"><span style="color:var(--muted)">Sin ingresos</span><span>—</span></div>`}
    ...
    ${topExp.map(([cat,val])=>{ const c=CATEGORIES.find(x=>x.id===cat)||{emoji:'📦',name:cat}; return `<div class="fs-row"><span>${c.emoji} ${c.name}</span><span style="color:var(--red)">${fmt(val)}</span></div>`; }).join('')||`<div class="fs-row"><span style="color:var(--muted)">Sin gastos</span><span>—</span></div>`}
  ...
  for (let i=5;i>=0;i--) {
    ...
    months6.push({ label:new Date(yy,mm,1).toLocaleDateString('es-MX',{month:'short'}), income:inc, expense:exp, net:inc-exp });
  }
  const maxV6 = Math.max(...months6.map(x=>Math.max(x.income,x.expense)),1);
  const svgH = 80, svgW = 300, barW = 20, gap = 10;
  const totalW = months6.length * (barW*2+gap+4);
  let svgBars = '';
  months6.forEach((mo,i)=>{
    const x0 = i*(barW*2+gap+4);
    const hInc = Math.max((mo.income/maxV6)*svgH,2);
    const hExp = Math.max((mo.expense/maxV6)*svgH,2);
    svgBars += `<rect x="${x0}" y="${svgH-hInc}" width="${barW}" height="${hInc}" rx="3" fill="#00e5a0" opacity="0.85"/>`;
    svgBars += `<rect x="${x0+barW+2}" y="${svgH-hExp}" width="${barW}" height="${hExp}" rx="3" fill="#ff4d6d" opacity="0.85"/>`;
    svgBars += `<text x="${x0+barW}" y="${svgH+14}" text-anchor="middle" fill="${mo.net>=0?'#00e5a0':'#ff4d6d'}" font-size="9" font-family="Syne,sans-serif">${mo.label}</text>`;
  });
  flowCard.innerHTML = `
    <div class="card-title">💹 Flujo de caja — 6 meses</div>
    <svg viewBox="0 0 ${totalW} ${svgH+20}" style="width:100%;overflow:visible">${svgBars}</svg>
  ...
  // ── DONA GASTOS ──
  if (Object.keys(byExpenseCat).length > 0) {
    const donutCard = document.createElement('div');
    donutCard.className = 'card';
    const donutColors = ['#00d4ff','#ff4d6d','#00e5a0','#ffc857','#b57bee','#f97316','#38bdf8','#fb7185'];
    const total = Object.values(byExpenseCat).reduce((s,v)=>s+v,0);
    const sorted = Object.entries(byExpenseCat).sort((a,b)=>b[1]-a[1]).slice(0,6);
    let startAngle = -90;
    let paths = '';
    sorted.forEach(([cat,val],i)=>{
      ...
      paths += `<path d="M${cx},${cy} L${x1},${y1} A${r},${r} 0 ${large},1 ${x2},${y2} Z" fill="${color}" opacity="0.9" stroke="var(--card)" stroke-width="1.5"/>`;
      startAngle += angle;
    });
    const legend = sorted.map(([cat,val],i)=>{
      const c = CATEGORIES.find(x=>x.id===cat)||{emoji:'📦',name:cat};
      return `<div style="display:flex;align-items:center;gap:6px;font-size:11px;margin-bottom:5px"><span style="width:10px;height:10px;border-radius:2px;background:${donutColors[i%donutColors.length]};flex-shrink:0"></span><span style="color:var(--muted2)">${c.emoji} ${c.name}</span><span style="margin-left:auto;font-family:'JetBrains Mono',monospace;color:var(--text)">${Math.round((val/total)*100)}%</span></div>`;
    }).join('');
    donutCard.innerHTML = `
      <div class="card-title">🍩 Distribución de gastos</div>
      <div style="display:flex;align-items:center;gap:16px">
        <svg viewBox="0 0 120 120" style="width:110px;flex-shrink:0">${paths}</svg>
  ...
  const debtToInc  = income>0 ? ((state.debts.reduce((s,d)=>s+d.monthly,0)/income)*100).toFixed(1) : 0;
  const debtToAss  = totalAssets>0 ? ((totalDebt/totalAssets)*100).toFixed(1) : 0;
  const indicators = [
    { label:'Tasa de ahorro',      value:`${savingRate}%`,   good:parseFloat(savingRate)>=20,    tip:'Ideal: >20%' },
    { label:'Deuda / Ingresos',    value:`${debtToInc}%`,    good:parseFloat(debtToInc)<=35,     tip:'Ideal: <35%' },
    { label:'Deuda / Activos',     value:`${debtToAss}%`,    good:parseFloat(debtToAss)<=50,     tip:'Ideal: <50%' },
    { label:'Rentabilidad invest.', value:`${investCost>0?((investGain/investCost)*100).toFixed(1):0}%`, good:investGain>=0, tip:'Ganancia total' },
  ];
  kpiCard.innerHTML = `
    <div class="card-title">📐 Indicadores clave</div>
    ${indicators.map(ind=>`
      <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border)">
        <div><div style="font-size:13px;font-weight:600">${ind.label}</div><div style="font-size:11px;color:var(--muted)">${ind.tip}</div></div>
        <div style="text-align:right"><div style="font-family:'JetBrains Mono',monospace;font-size:16px;font-weight:700;color:${ind.good?'var(--green)':'var(--red)'}">${ind.value}</div><div style="font-size:10px;color:${ind.good?'var(--green)':'var(--red)'}">${ind.good?'✅ Saludable':'⚠️ Revisar'}</div></div>
      </div>`).join('')}`;
...
function renderInvestTypeGrid() {
  ...
  grid.innerHTML=INVEST_BY_ZONE[state.selectedZone].map(t=>`<button class="cat-btn ${state.selectedInvestType===t.id?'selected':''}" data-itype="${t.id}"><span class="cat-btn-emoji">${t.emoji}</span><span class="cat-btn-name">${t.name}</span></button>`).join('');
...
function saveInvestment() {
  ...
  if(!name){toast('Ingresa un nombre');return;}
  if(cost<=0){toast('Ingresa el monto invertido');return;}
  ...
  if(state.editingInvestId){
    ...
    // Registrar como gasto para descontar del saldo líquido
    state.transactions.push({id:uid(),type:'gasto',amount:cost,desc:`Inversión: ${name}`,category:'inversion_out',date:todayStr()});
  }
...
function openSettings() {
  ...
  document.getElementById('budget-inputs').innerHTML=CATEGORIES.map(c=>`<div class="budget-inp-row"><div class="budget-inp-label">${c.emoji} ${c.name}</div><input type="number" inputmode="decimal" placeholder="0" value="${state.settings.budgets[c.id]||''}" data-budget="${c.id}" /></div>`).join('');
}
...
function init() {
  ...
  document.querySelectorAll('.tri-btn').forEach(b=>b.addEventListener('click',()=>{ state.selectedZone=b.dataset.zone; state.selectedInvestType=INVEST_BY_ZONE[state.selectedZone][0].id; updateTriangleSelector(); renderInvestTypeGrid(); }));
  ...
  document.querySelectorAll('[data-profile]').forEach(b=>b.addEventListener('click',()=>{ state.settings.investorProfile=b.dataset.profile; document.querySelectorAll('[data-profile]').forEach(x=>x.classList.toggle('active',x.dataset.profile===b.dataset.profile)); }));
  document.getElementById('btn-clear-data').addEventListener('click',()=>{ if(confirm('¿Borrar TODOS los datos? No se puede deshacer.')){ localStorage.clear(); location.reload(); } });
